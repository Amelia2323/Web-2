...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cek->execute([$_POST['dosen_id'], $_POST['kegiatan_id']]);
    $sudahAda = $cek->fetchColumn();

    if ($sudahAda) {
        $error = "Data dosen dan kegiatan ini sudah terdaftar.";
    } else {
        $stmt = $dbh->prepare("INSERT INTO dosen_kegiatan (dosen_id, kegiatan_id) VALUES (?, ?)");
        $stmt->execute([$_POST['dosen_id'], $_POST['kegiatan_id']]);
...
<body class="sb-nav-fixed">
    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
        <!-- Navbar Brand-->
        <a class="navbar-brand ps-3" href="admin/index.php">Dosen Kegiatan</a>
        <!-- Sidebar Toggle-->
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    ...
                    <h2 class="mt-4">Tambah Dosen Kegiatan</h2>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="kegiatan_id" class="form-control" required>
                                        ...
                                        while ($k = $kegiatan->fetch()) {
                                            echo "<option value='{$k['id']}'>{$k['tempat']} ({$k['tanggal_mulai']})</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
